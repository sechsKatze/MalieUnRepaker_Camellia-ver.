...
def main(args=None):
    ...
    try:
        if len(args) < 2:
            print("사용법: python unpack_plain.py <input.dat> <output_file> [--keep-padding]")
            return

        input_path = args[0]
        output_path = args[1]   # CLI 두 번째 인자로 출력 파일 지정
        keep_padding = "--keep-padding" in args
        ...
        # suffix는 CLI로 직접 output_path 지정 시 무시해도 되지만,
        # 옵션에 따라 덮어쓸 수 있도록 유지
        if output_path.endswith(".dat") is False:
            suffix = "_plain_full.dat" if keep_padding else "_plain.dat"
            output_path = input_path.replace(".dat", suffix)

        start = time.time()
        decrypt_full_dat(archive, output_path, keep_padding=keep_padding)
