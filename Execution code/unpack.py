# unpack.py - 말리 엔진 전체 포맷 테스트 + 복호화 실행
...
def write_entry_list(archive, output_path="entry_list.txt"):
    with open(output_path, "w", encoding="utf-8") as f:
        for entry in getattr(archive, "dir_with_dirs", archive.entries):
            is_dir = getattr(entry, "is_dir", False)
            line = (
                f"<AutoEntry name={entry.name}, "
                f"entry_index={getattr(entry, 'entry_index', 'N/A')}, "
                f"offset_index={getattr(entry, 'offset_index', 'N/A')}, "
                f"offset=0x{entry.offset:X}, size=0x{entry.size:X}, "
                f"is_dir={is_dir}>\n"
            )
            f.write(line)
    print(f"[✓] Entry 리스트 저장됨 → {output_path}")
...
def process_png_file(entry, save_path_base):
    try:
        ...
        # .pn은 글자 제한 때문에 잘린 .png일 수 있으므로 .png로 저장
...
def decrypt_png_stream(entry):
    try:
        ...
        #시그니처가 MalieGF면 decrypt_png_malie로 분기
        if prefix.startswith(b'MalieGF'):
            return decrypt_png_malie(entry, buf)
        #시그니처가 PNG면 decrypt_png_normal로 분기
...
#MalieGF가 감지되면 MGF로 분기
def decrypt_png_malie(entry, data):
    try:
        stream = PrefixStream(b"MalieGF", io.BytesIO(data[7:])) 
        ...
        if not metadata:
            return None
        stream.seek(0)
        return handler.read(stream, metadata)  # read_auto 안 써도 됨

    except Exception as e:
        logging.error("[decrypt_png_malie] 예외 발생", exc_info=True)
        return None
...
def main():
    ...
    try:
        if len(sys.argv) < 2:
            print("사용법: python unpack_test.py [대상 .dat 파일 경로]")
            return
        ...
        if archive:
            logging.debug("[unpack_test] LibOpener 성공")
        else:
            logging.debug("[unpack_test] LibOpener 실패 → DatOpener 시도")
            view.close()
            view = FileView(input_path)
            archive = DatOpener().try_open(view)
            if archive:
                print(f"[DatOpener] entries 개수: {len(archive.entries)}")
                logging.debug("[unpack_test] DatOpener 성공")
            else:
                logging.error("[unpack_test] DatOpener 실패 → 아카이브 열기 실패")
                return
            
        write_entry_list(archive, "game_entry_list.txt")
        ...
        if os.path.isfile(json_path):
            logging.info(f"[unpack_test] JSON 메타데이터 불러오기: {json_path}")
            meta_manager.apply_to_entries(archive.entries)
        else:
            logging.info(f"[unpack_test] JSON 메타데이터 파일이 없어 새로 생성합니다: {json_path}")

        # 항상 JSON 메타데이터 저장 (새로 생성 또는 덮어쓰기)
        meta_manager.save_metadata(archive.entries)

        # 디버깅용: 상위 10개 엔트리 정보 출력
        logging.info("[unpack_test] JSON 메타데이터 적용 완료, 엔트리 일부 확인:")
        for i, entry in enumerate(archive.entries[:10]):
            if isinstance(entry, dict):
                name = entry.get('name') or entry.get('arc_path', 'Unknown')
                offset = entry.get('offset', None)
            else:
                name = getattr(entry, 'name', getattr(entry, 'arc_path', 'Unknown'))
                offset = getattr(entry, 'offset', None)
            logging.info(f"  [{i:03d}] name: {name}, offset: {offset}")

        # ✅ 복호화 시간 측정 시작
        start = time.time()
        run_serial_unpack(archive, view, output_dir)  
        elapsed = time.time() - start
        print(f"[완료] 전체 복호화 시간: {elapsed:.2f}초")
        
        logging.info("[unpack_test] 전체 엔트리 목록:")
        for i, entry in enumerate(archive.entries):
            logging.info(f"  [{i:04d}] {entry}")
